...
/**
 * Обработчик создания краевых линий
 * Адаптированная версия MouseHandlerCreaseMakeEdge для Android
 */
class MouseHandlerCreaseMakeEdge : BaseMouseHandler() {
    
    private var startPoint: Point? = null
    private var endPoint: Point? = null
    private var isSelecting = false
    private var convertedLines = mutableListOf<LineSegment>()
    private var createdEdgeLines = mutableListOf<LineSegment>()
    ...
    override fun onPress(offset: Offset, change: PointerInputChange): Boolean {
        ...
        println("Начало создания краевых линий в точке: (${point.x}, ${point.y})")
    ...
    private fun performSingleConversion(point: Point) {
        ...
        if (closestLine != null && point.distance(getLineCenter(closestLine)) < getSelectionDistance()) {
            // Преобразовать линию в краевую (черную)
            val edgeLine = LineSegment(closestLine.a, closestLine.b, LineColor.BLACK_0)
            
            // Установить цвет линии
            setLineColor(closestLine, LineColor.BLACK_0)
            
            convertedLines.add(closestLine)
            createdEdgeLines.add(edgeLine)
            
            println("Преобразована линия в краевую: (${closestLine.determineAX()}, ${closestLine.determineAY()}) -> (${closestLine.determineBX()}, ${closestLine.determineBY()})")
        }
    }
    
    private fun performRectangleConversion(start: Point, end: Point) {
        // Преобразовать линии в прямоугольнике в краевые
        val linesInBox = findLinesInBox(start, end)
        var convertedCount = 0
        
        for (line in linesInBox) {
            // Преобразовать линию в краевую (черную)
            val edgeLine = LineSegment(line.a, line.b, LineColor.BLACK_0)
            ...
            println("Преобразована линия в краевую: (${line.determineAX()}, ${line.determineAY()}) -> (${line.determineBX()}, ${line.determineBY()})")
        }
    ...
    fun getCreatedEdgeLines(): List<LineSegment> = createdEdgeLines.toList()
    
    fun getConvertedCount(): Int = convertedLines.size
    
    fun getCreatedCount(): Int = createdEdgeLines.size
    ...
    fun clearCreatedEdgeLines() {
        createdEdgeLines.clear()
    }
    
    fun getConversionDescription(): String {
        return "Преобразовано ${convertedLines.size} линий в краевые"
    }
    
    fun getEdgeLineDescription(): String {
        return "Краевые линии (черные) используются для обозначения краев бумаги"
    }
    
    override fun getName(): String = "Создание краевых линий"
    
    override fun getDescription(): String = "Преобразуйте линии в краевые (черные)"
} 