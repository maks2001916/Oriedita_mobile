...
/**
 * Обработчик отображения угла между тремя точками (тип 2)
 * Адаптированная версия для Android
 */
class MouseHandlerDisplayAngleBetweenThreePoints2 : BaseMouseHandler() {
    ...
    private fun calculateAngle() {
        if (getLineStepSize() >= 3) {
            ...
            println("Измерен угол (тип 2): ${Math.toDegrees(measuredAngle)}° (${measuredAngle} радиан)")
    ...
    fun getAngleInfo(): String {
        return buildString {
            ...
            append("Измеренный угол (тип 2): ${String.format("%.2f", Math.toDegrees(measuredAngle))}°\n")
            append("В радианах: ${String.format("%.4f", measuredAngle)}")
        }
    }
    
    override fun getName(): String = "Измерение угла между тремя точками (тип 2)"
    
    override fun getDescription(): String = "Измеряйте углы между тремя выбранными точками (второй тип)"
} 