...
/**
 * Обработчик измерения расстояния между точками (тип 1)
 * Адаптированная версия для Android
 */
class MouseHandlerDisplayLengthBetweenPoints1 : BaseMouseHandler() {
    ...
    private fun calculateLength() {
        if (getLineStepSize() >= 2) {
            ...
            println("Измерено расстояние (тип 1): ${String.format("%.4f", measuredLength)}")
    ...
    fun getLengthInfo(): String {
        ...
        return buildString {
            ...
            append("Измеренное расстояние (тип 1): ${String.format("%.4f", measuredLength)}\n")
            append("Размер сетки: $gridSize")
        }
    }
    
    override fun getName(): String = "Измерение расстояния между точками (тип 1)"
    
    override fun getDescription(): String = "Измеряйте расстояния между двумя выбранными точками"
} 