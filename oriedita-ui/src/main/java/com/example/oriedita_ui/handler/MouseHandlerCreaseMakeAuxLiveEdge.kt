...
/**
 * Обработчик создания вспомогательных живых краевых линий
 * Адаптированная версия для Android
 */
class MouseHandlerCreaseMakeAuxLiveEdge : BaseMouseHandler() {
    
    private var startPoint: Point? = null
    private var endPoint: Point? = null
    private var isSelecting = false
    private var convertedLines = mutableListOf<LineSegment>()
    private var createdAuxLiveEdgeLines = mutableListOf<LineSegment>()
    ...
    override fun onPress(offset: Offset, change: PointerInputChange): Boolean {
        ...
        println("Начало создания вспомогательных живых краевых линий в точке: (${point.x}, ${point.y})")
    ...
    private fun performSingleConversion(point: Point) {
        ...
        if (closestLine != null && point.distance(getLineCenter(closestLine)) < getSelectionDistance()) {
            // Преобразовать линию в вспомогательную живую краевую (черную)
            val auxLiveEdgeLine = LineSegment(closestLine.getA(), closestLine.getB(), LineColor.BLACK_0)
            
            // Установить цвет линии
            setLineColor(closestLine, LineColor.BLACK_0)
            
            convertedLines.add(closestLine)
            createdAuxLiveEdgeLines.add(auxLiveEdgeLine)
            
            println("Преобразована линия в вспомогательную живую краевую: (${closestLine.determineAX()}, ${closestLine.determineAY()}) -> (${closestLine.determineBX()}, ${closestLine.determineBY()})")
        }
    }
    
    private fun performRectangleConversion(start: Point, end: Point) {
        // Преобразовать линии в прямоугольнике в вспомогательные живые краевые
        val linesInBox = findLinesInBox(start, end)
        var convertedCount = 0
        
        for (line in linesInBox) {
            // Преобразовать линию в вспомогательную живую краевую (черную)
            val auxLiveEdgeLine = LineSegment(line.getA(), line.getB(), LineColor.BLACK_0)
            ...
            println("Преобразована линия в вспомогательную живую краевую: (${line.determineAX()}, ${line.determineAY()}) -> (${line.determineBX()}, ${line.determineBY()})")
        }
    ...
    fun getCreatedAuxLiveEdgeLines(): List<LineSegment> = createdAuxLiveEdgeLines.toList()
    
    fun getConvertedCount(): Int = convertedLines.size
    
    fun getCreatedCount(): Int = createdAuxLiveEdgeLines.size
    ...
    fun clearCreatedAuxLiveEdgeLines() {
        createdAuxLiveEdgeLines.clear()
    }
    
    fun getConversionDescription(): String {
        return "Преобразовано ${convertedLines.size} линий в вспомогательные живые краевые"
    }
    
    fun getAuxLiveEdgeLineDescription(): String {
        return "Вспомогательные живые краевые линии (черные) используются для временных краевых построений"
    }
    
    override fun getName(): String = "Создание вспомогательных живых краевых линий"
    
    override fun getDescription(): String = "Преобразуйте линии в вспомогательные живые краевые (черные)"
} 