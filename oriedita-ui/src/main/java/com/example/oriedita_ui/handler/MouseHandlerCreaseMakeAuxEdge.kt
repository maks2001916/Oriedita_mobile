...
/**
 * Обработчик создания вспомогательных краевых линий
 * Адаптированная версия для Android
 */
class MouseHandlerCreaseMakeAuxEdge : BaseMouseHandler() {
    
    private var startPoint: Point? = null
    private var endPoint: Point? = null
    private var isSelecting = false
    private var convertedLines = mutableListOf<LineSegment>()
    private var createdAuxEdgeLines = mutableListOf<LineSegment>()
    ...
    override fun onPress(offset: Offset, change: PointerInputChange): Boolean {
        ...
        println("Начало создания вспомогательных краевых линий в точке: (${point.x}, ${point.y})")
    ...
    private fun performSingleConversion(point: Point) {
        ...
        if (closestLine != null && point.distance(getLineCenter(closestLine)) < getSelectionDistance()) {
            // Преобразовать линию в вспомогательную краевую (черную)
            val auxEdgeLine = LineSegment(closestLine.a, closestLine.b, LineColor.BLACK_0)

            // Установить цвет линии
            setLineColor(closestLine, LineColor.BLACK_0)
            
            convertedLines.add(closestLine)
            createdAuxEdgeLines.add(auxEdgeLine)
            
            println("Преобразована линия в вспомогательную краевую: (${closestLine.determineAX()}, ${closestLine.determineAY()}) -> (${closestLine.determineBX()}, ${closestLine.determineBY()})")
        }
    }
    
    private fun performRectangleConversion(start: Point, end: Point) {
        // Преобразовать линии в прямоугольнике в вспомогательные краевые
        val linesInBox = findLinesInBox(start, end)
        var convertedCount = 0
        
        for (line in linesInBox) {
            // Преобразовать линию в вспомогательную краевую (черную)
            val auxEdgeLine = LineSegment(line.a, line.b, LineColor.BLACK_0)
            ...
            println("Преобразована линия в вспомогательную краевую: (${line.determineAX()}, ${line.determineAY()}) -> (${line.determineBX()}, ${line.determineBY()})")
        }
    ...
    fun getCreatedAuxEdgeLines(): List<LineSegment> = createdAuxEdgeLines.toList()
    
    fun getConvertedCount(): Int = convertedLines.size
    
    fun getCreatedCount(): Int = createdAuxEdgeLines.size
    ...
    fun clearCreatedAuxEdgeLines() {
        createdAuxEdgeLines.clear()
    }
    
    fun getConversionDescription(): String {
        return "Преобразовано ${convertedLines.size} линий в вспомогательные краевые"
    }
    
    fun getAuxEdgeLineDescription(): String {
        return "Вспомогательные краевые линии (черные) используются для обозначения краев в вспомогательных построениях"
    }
    
    override fun getName(): String = "Создание вспомогательных краевых линий"
    
    override fun getDescription(): String = "Преобразуйте линии в вспомогательные краевые (черные)"
}