...
/**
 * Обработчик создания вспомогательных живых горных линий
 * Адаптированная версия для Android
 */
class MouseHandlerCreaseMakeAuxLiveMountain : BaseMouseHandler() {
    
    private var startPoint: Point? = null
    private var endPoint: Point? = null
    private var isSelecting = false
    private var convertedLines = mutableListOf<LineSegment>()
    private var createdAuxLiveMountainLines = mutableListOf<LineSegment>()
    ...
    override fun onPress(offset: Offset, change: PointerInputChange): Boolean {
        ...
        println("Начало создания вспомогательных живых горных линий в точке: (${point.x}, ${point.y})")
    ...
    private fun performSingleConversion(point: Point) {
        ...
        if (closestLine != null && point.distance(getLineCenter(closestLine)) < getSelectionDistance()) {
            // Преобразовать линию в вспомогательную живую горную (красную)
            val auxLiveMountainLine = LineSegment(closestLine.a, closestLine.b,LineColor.RED_1)
            
            // Установить цвет линии
            setLineColor(closestLine, LineColor.RED_1)
            
            convertedLines.add(closestLine)
            createdAuxLiveMountainLines.add(auxLiveMountainLine)
            
            println("Преобразована линия в вспомогательную живую горную: (${closestLine.determineAX()}, ${closestLine.determineAY()}) -> (${closestLine.determineBX()}, ${closestLine.determineBY()})")
        }
    }
    
    private fun performRectangleConversion(start: Point, end: Point) {
        // Преобразовать линии в прямоугольнике в вспомогательные живые горные
        val linesInBox = findLinesInBox(start, end)
        var convertedCount = 0
        
        for (line in linesInBox) {
            // Преобразовать линию в вспомогательную живую горную (красную)
            val auxLiveMountainLine = LineSegment(line.a, line.b, LineColor.RED_1)
            ...
            println("Преобразована линия в вспомогательную живую горную: (${line.determineAX()}, ${line.determineAY()}) -> (${line.determineBX()}, ${line.determineBY()})")
        }
    ...
    fun getCreatedAuxLiveMountainLines(): List<LineSegment> = createdAuxLiveMountainLines.toList()
    
    fun getConvertedCount(): Int = convertedLines.size
    
    fun getCreatedCount(): Int = createdAuxLiveMountainLines.size
    ...
    fun clearCreatedAuxLiveMountainLines() {
        createdAuxLiveMountainLines.clear()
    }
    
    fun getConversionDescription(): String {
        return "Преобразовано ${convertedLines.size} линий в вспомогательные живые горные"
    }
    
    fun getAuxLiveMountainLineDescription(): String {
        return "Вспомогательные живые горные линии (красные) используются для временных горных построений"
    }
    
    override fun getName(): String = "Создание вспомогательных живых горных линий"
    
    override fun getDescription(): String = "Преобразуйте линии в вспомогательные живые горные (красные)"
} 