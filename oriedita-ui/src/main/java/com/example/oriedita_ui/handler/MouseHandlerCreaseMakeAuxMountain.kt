...
/**
 * Обработчик создания вспомогательных горных линий
 * Адаптированная версия для Android
 */
class MouseHandlerCreaseMakeAuxMountain : BaseMouseHandler() {
    
    private var startPoint: Point? = null
    private var endPoint: Point? = null
    private var isSelecting = false
    private var convertedLines = mutableListOf<LineSegment>()
    private var createdAuxMountainLines = mutableListOf<LineSegment>()
    ...
    override fun onPress(offset: Offset, change: PointerInputChange): Boolean {
        ...
        println("Начало создания вспомогательных горных линий в точке: (${point.x}, ${point.y})")
    ...
    private fun performSingleConversion(point: Point) {
        ...
        if (closestLine != null && point.distance(getLineCenter(closestLine)) < getSelectionDistance()) {
            // Преобразовать линию в вспомогательную горную (красную)
            val auxMountainLine = LineSegment(closestLine.a, closestLine.b, LineColor.RED_1)

            // Установить цвет линии
            setLineColor(closestLine, LineColor.RED_1)
            
            convertedLines.add(closestLine)
            createdAuxMountainLines.add(auxMountainLine)
            
            println("Преобразована линия в вспомогательную горную: (${closestLine.determineAX()}, ${closestLine.determineAY()}) -> (${closestLine.determineBX()}, ${closestLine.determineBY()})")
        }
    }
    
    private fun performRectangleConversion(start: Point, end: Point) {
        // Преобразовать линии в прямоугольнике в вспомогательные горные
        val linesInBox = findLinesInBox(start, end)
        var convertedCount = 0
        
        for (line in linesInBox) {
            // Преобразовать линию в вспомогательную горную (красную)
            val auxMountainLine = LineSegment(line.a, line.b, LineColor.RED_1)
            ...
            println("Преобразована линия в вспомогательную горную: (${line.determineAX()}, ${line.determineAY()}) -> (${line.determineBX()}, ${line.determineBY()})")
        }
    ...
    fun getCreatedAuxMountainLines(): List<LineSegment> = createdAuxMountainLines.toList()
    
    fun getConvertedCount(): Int = convertedLines.size
    
    fun getCreatedCount(): Int = createdAuxMountainLines.size
    ...
    fun clearCreatedAuxMountainLines() {
        createdAuxMountainLines.clear()
    }
    
    fun getConversionDescription(): String {
        return "Преобразовано ${convertedLines.size} линий в вспомогательные горные"
    }
    
    fun getAuxMountainLineDescription(): String {
        return "Вспомогательные горные линии (красные) используются для обозначения горных сгибов в вспомогательных построениях"
    }
    
    override fun getName(): String = "Создание вспомогательных горных линий"
    
    override fun getDescription(): String = "Преобразуйте линии в вспомогательные горные (красные)"
} 