...
/**
 * Обработчик отображения угла между тремя точками (тип 1)
 * Адаптированная версия для Android
 */
class MouseHandlerDisplayAngleBetweenThreePoints1 : BaseMouseHandler() {
    ...
    private fun calculateAngle() {
        if (getLineStepSize() >= 3) {
            ...
            println("Измерен угол: ${Math.toDegrees(measuredAngle)}° (${measuredAngle} радиан)")
    ...
    fun getAngleInfo(): String {
        return buildString {
            ...
            append("Измеренный угол: ${String.format("%.2f", Math.toDegrees(measuredAngle))}°\n")
            append("В радианах: ${String.format("%.4f", measuredAngle)}")
        }
    }
    
    override fun getName(): String = "Измерение угла между тремя точками (тип 1)"
    
    override fun getDescription(): String = "Измеряйте углы между тремя выбранными точками"
} 