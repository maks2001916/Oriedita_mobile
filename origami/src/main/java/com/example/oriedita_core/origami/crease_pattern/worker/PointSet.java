...
public class PointSet implements Serializable {
    ...
    private transient int[] lineInFaceBorder_min;
    private transient int[] lineInFaceBorder_max;

    private Face[] faces; //Face instantiation

    private transient List<List<Integer>> point_linking;//point_linking [i] [j] is the number of points connected to t [i]. The number of Tem is stored in t [0].
    ...
    public boolean calculateFaces() throws InterruptedException {
        Log.i("TAG","線分集合->点集合：点集合内で面を発生　開始");
        boolean addNewFace;
        Face tempFace;
        numFaces = 0;
        searchPointLinking();
        ...
        for (int i = 1; i <= numLines; i++) {
            ...
            if (addNewFace && tempFace.getNumPoints() != 0 && calculateArea(tempFace) > 0.0) {
                try {
                    ...
                    Log.e("TAG","ERROR: cannot add face.");
                    return false;
                }
            }
        ...
        if (euler != 1) {
             ...
             */
            Log.w("TAG","Euler characteristic error");
